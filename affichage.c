...
void dessiner_ligne_score(int ligne, int colonne, int nombre, int couleur)// générateur de carrés en ligne
{
    int largeur_rect = 6;
    int hauteur_rect = 3;
    int espace = 4;
...
void afficher_haie(S_jeu* game, S_joueur joueur[])
{
    ...
    for (int lig = 0; lig < nb_lignes; lig++)// meme base generatrice modulaire pour faire tous les trucs à case
    {
...
void victoire()
{
    ...
    printf("| .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. |");
    positionner_curseur(l + 2, c);
    printf("| | ____    ____ | || |     _____    | || |      ______  | || |  _________   | || |     ____     | || |     _____    | || |  _______     | || |  _________   | |");
    positionner_curseur(l + 3, c);
    printf("| ||_  _| |_  _| | || |    |_   _|   | || |   .' ___  |  | || | |  _   _  |  | || |   .'    `.   | || |    |_   _|   | || | |_   __ \\    | || | |_   ___  |  | |");
    positionner_curseur(l + 4, c);
    printf("| |  \\ \\   / /   | || |      | |     | || |  / .'   \\_|  | || | |_/ | | \\_|  | || |  /  .--.  \\  | || |      | |     | || |   | |__) |   | || |   | |_  \\_|  | |");
    positionner_curseur(l + 5, c);
    printf("| |   \\ \\ / /    | || |      | |     | || |  | |         | || |     | |      | || |  | |    | |  | || |      | |     | || |   |  __ /    | || |   |  _|  _   | |");
    positionner_curseur(l + 6, c);
    printf("| |    \\ ' /     | || |     _| |_    | || |  \\ `.___.'\\  | || |    _| |_     | || |  \\  `--'  /  | || |     _| |_    | || |  _| |  \\ \\_  | || |  _| |___/ |  | |");
    positionner_curseur(l + 7, c);
    printf("| |     \\_/      | || |    |_____|   | || |   `._____.'  | || |   |_____|    | || |   `.____.'   | || |    |_____|   | || | |____| |___| | || | |_________|  | |");
    positionner_curseur(l + 8, c);
    printf("| |              | || |              | || |              | || |              | || |              | || |              | || |              | || |              | |");
    positionner_curseur(l + 9, c);
    printf("| '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' |");
    positionner_curseur(l + 10, c);
    printf(" '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------' ");
}
...
{
    HANDLE hStdout = GetStdHandle(STD_OUTPUT_HANDLE);//permet de déclarer la variable "hstdout" qui fait référence à la console
